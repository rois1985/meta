...
async function performUrlScan(urlConfig) {
  ...
  try {
    // Attempt to fetch the URL with realistic browser headers
    ...
    if (response.status >= 400) {
      return {
        ...
        status: 'blocked',
        error: `Website returned status ${response.status} - access may be restricted`,
        overallScore: null,
        violations: [],
        warnings: [{ title: 'Access Restricted', description: `The website blocked our scan request (HTTP ${response.status}). This is common for sites with bot protection.` }]
      };
    }
